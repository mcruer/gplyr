...
test_that("filter_str_in keeps matching rows", {
  result <- filter_str_in(df, role, "Data")
  expect_equal(nrow(result), 2)
  expect_true(all(grepl("Data", result$role)))
})

test_that("filter_str_out removes matching rows", {
  result <- filter_str_out(df, role, "Data")
  expect_equal(nrow(result), 2)
  expect_false(any(grepl("Data", result$role, ignore.case = TRUE), na.rm = TRUE))
})

test_that("filter_str_in then filter_str_out works in a pipe chain", {
  result <- df |> filter_str_in(role, "Data") |> filter_str_out(role, "financial")
  expect_equal(nrow(result), 2)
})

test_that("filter_str_out preserves NA rows by default", {
  result <- filter_str_out(df, role, "Data")
  expect_true(any(is.na(result$role)))
})

test_that("filter_str_out drops NA rows when na.rm = TRUE", {
  result <- filter_str_out(df, role, "Data", na.rm = TRUE)
